...
import type { IMembershipTypeAdapter } from '../membershipType.adapter';


@injectable()
export class MembershipTypeApiAdapter
  extends ApiBaseAdapter<MembershipType>
  implements IMembershipTypeAdapter
{
  protected basePath = '/membershiptypes';
  protected mapFromApi(data: any): MembershipType {
    return {
      id: data.id ?? data.Id,
  ...
  protected mapToApi(data: Partial<MembershipType>) {
    return {
      id: data.id,
      code: data.code,
      name: data.name,
