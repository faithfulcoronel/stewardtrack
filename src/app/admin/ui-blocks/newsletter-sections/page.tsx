...
export const metadata: Metadata = {
  title: "Newsletter sections | StewardTrack",
};
...
export default async function NewsletterSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Newsletter sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate newsletter samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectNewsletterSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Newsletter sections</h1>
          <p className="text-sm text-muted-foreground">
            No newsletter blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to see them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Newsletter sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable newsletter blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectNewsletterSamples(manifest: ManifestFile): Promise<NewsletterSample[]> {
  const newsletterEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("newsletter/") &&
        (entry.layer.tenant === null || entry.layer.tenant === undefined)
    )
    .sort((a, b) => a.layer.route.localeCompare(b.layer.route));

  const samples: NewsletterSample[] = [];

  for (const entry of newsletterEntries) {
    const details = await readNewsletterDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block newsletter - ${fallbackName}`,
...
async function readNewsletterDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
