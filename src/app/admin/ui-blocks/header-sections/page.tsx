...
export const metadata: Metadata = {
  title: "Header sections | StewardTrack",
};
...
export default async function HeaderSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Header sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate header samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectHeaderSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Header sections</h1>
          <p className="text-sm text-muted-foreground">
            No header blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to see them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Header sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable header blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectHeaderSamples(manifest: ManifestFile): Promise<HeaderSample[]> {
  const headerEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("header/") &&
  ...
  for (const entry of headerEntries) {
    const details = await readHeaderDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block header - ${fallbackName}`,
      variant: details.variant ?? "gradient-orbit",
...
async function readHeaderDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
