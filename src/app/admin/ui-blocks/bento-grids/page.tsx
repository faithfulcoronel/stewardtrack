...
export default async function BentoGridsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Bento grids</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate bento grid samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectBentoSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Bento grids</h1>
          <p className="text-sm text-muted-foreground">
            No bento grid blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to see
            them here.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Bento grids</h1>
        <p className="text-sm text-muted-foreground">
          Explore multi-card feature grids compiled from XML blueprints. Click a card to open the rendered page in a new tab.
        </p>
      </div>
          ...
          <Card key={sample.key} className="border-border/60">
            ...
            <CardContent className="space-y-4 text-sm text-muted-foreground">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="capitalize">Variant: {formatVariantLabel(sample.variant)}</Badge>
              </div>
...
async function collectBentoSamples(manifest: ManifestFile): Promise<BentoSample[]> {
  const bentoEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("bento/") &&
        (entry.layer.tenant === null || entry.layer.tenant === undefined)
    )
    .sort((a, b) => a.layer.route.localeCompare(b.layer.route));

  const samples: BentoSample[] = [];

  for (const entry of bentoEntries) {
    const details = await readBentoDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block bento grid - ${fallbackName}`,
      variant: details.variant ?? "product-orbit",
...
async function readBentoDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
