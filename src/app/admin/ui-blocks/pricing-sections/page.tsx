...
export const metadata: Metadata = {
  title: "Pricing sections | StewardTrack",
};

const ROOT = process.cwd();

interface PricingSample {
  key: string;
  name: string;
  title: string;
  route: string;
...
export default async function PricingSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Pricing sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate pricing samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectPricingSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Pricing sections</h1>
          <p className="text-sm text-muted-foreground">
            No pricing blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to view them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Pricing sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable pricing blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectPricingSamples(manifest: ManifestFile): Promise<PricingSample[]> {
  const pricingEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("pricing/") &&
        (entry.layer.tenant === null || entry.layer.tenant === undefined)
    )
    .sort((a, b) => a.layer.route.localeCompare(b.layer.route));

  const samples: PricingSample[] = [];

  for (const entry of pricingEntries) {
    const details = await readPricingDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block pricing - ${fallbackName}`,
      variant: details.variant ?? "three-column-highlight",
...
async function readPricingDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
