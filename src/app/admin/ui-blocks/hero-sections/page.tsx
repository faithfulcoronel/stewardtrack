...
export const metadata: Metadata = {
  title: "Hero sections | StewardTrack",
};
...
export default async function HeroSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Hero sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate hero samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectHeroSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Hero sections</h1>
          <p className="text-sm text-muted-foreground">
            No hero blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to see them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Hero sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable hero blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
          ...
          <Card key={sample.key} className="border-border/60">
            ...
            <CardContent className="space-y-4 text-sm text-muted-foreground">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="font-mono uppercase">Schema v{sample.schemaVersion}</Badge>
...
async function collectHeroSamples(manifest: ManifestFile): Promise<HeroSample[]> {
  const heroEntries = Object.values(manifest.entries)
    .filter((entry) =>
      entry.kind === "blueprint" &&
      entry.layer.module === "ui-blocks" &&
      entry.layer.route.startsWith("hero/") &&
      (entry.layer.tenant === null || entry.layer.tenant === undefined)
    )
    .sort((a, b) => a.layer.route.localeCompare(b.layer.route));

  const samples: HeroSample[] = [];

  for (const entry of heroEntries) {
    const details = await readHeroDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block hero - ${fallbackName}`,
      variant: details.variant ?? "aurora",
...
async function readHeroDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
