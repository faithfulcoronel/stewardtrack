...
export default async function FeatureSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Feature sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate feature section samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectFeatureSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Feature sections</h1>
          <p className="text-sm text-muted-foreground">
            No feature blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to see them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Feature sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable feature section blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectFeatureSamples(manifest: ManifestFile): Promise<FeatureSample[]> {
  const featureEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("feature/") &&
  ...
  for (const entry of featureEntries) {
    const details = await readFeatureDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block feature - ${fallbackName}`,
      variant: details.variant ?? "icon-grid",
...
async function readFeatureDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
