...
export const metadata: Metadata = {
  title: "Stats sections | StewardTrack",
};
...
export default async function StatsSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Stats sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate stats samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectStatsSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Stats sections</h1>
          <p className="text-sm text-muted-foreground">
            No stats blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to explore them here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Stats sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable stats blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectStatsSamples(manifest: ManifestFile): Promise<StatsSample[]> {
  const statsEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("stats/") &&
  ...
  for (const entry of statsEntries) {
    const details = await readStatsDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block stats - ${fallbackName}`,
      variant: details.variant ?? "glow-grid",
...
async function readStatsDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
