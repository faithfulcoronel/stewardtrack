...
export const metadata: Metadata = {
  title: "Testimonials sections | StewardTrack",
};
...
export default async function TestimonialsSectionsPage() {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Testimonials sections</h1>
          <p className="text-sm text-muted-foreground">
            The metadata registry manifest is empty. Run the metadata compiler to populate testimonial samples.
          </p>
        </div>
      </div>
    );
  ...
  const samples = await collectTestimonialSamples(manifest);
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold">Testimonials sections</h1>
          <p className="text-sm text-muted-foreground">
            No testimonial blueprints were discovered under the ui-blocks module. Add XML definitions and recompile metadata to
            see them here.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Testimonials sections</h1>
        <p className="text-sm text-muted-foreground">
          Explore reusable testimonial blueprints defined entirely in XML. Click a card to open the rendered page in a new tab.
        </p>
      </div>
...
async function collectTestimonialSamples(manifest: ManifestFile): Promise<TestimonialSample[]> {
  const testimonialEntries = Object.values(manifest.entries)
    .filter(
      (entry) =>
        entry.kind === "blueprint" &&
        entry.layer.module === "ui-blocks" &&
        entry.layer.route.startsWith("testimonials/") &&
        (entry.layer.tenant === null || entry.layer.tenant === undefined)
    )
    .sort((a, b) => a.layer.route.localeCompare(b.layer.route));

  const samples: TestimonialSample[] = [];

  for (const entry of testimonialEntries) {
    const details = await readTestimonialDetails(entry);
    const fallbackName = formatRouteLabel(entry.layer.route);
    samples.push({
      key: entry.key,
      name: fallbackName,
      title: details.title ?? `UI Block testimonials - ${fallbackName}`,
      variant: details.variant ?? "spotlight-carousel",
...
async function readTestimonialDetails(entry: ManifestEntry): Promise<{ title: string | null; variant: string | null }> {
