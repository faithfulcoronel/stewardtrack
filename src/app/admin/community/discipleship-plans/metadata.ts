/**
 * ================================================================================
 * DISCIPLESHIP PLANS METADATA RENDERER
 * ================================================================================
 *
 * This file provides the metadata rendering function for discipleship plan pages.
 * It serves as the bridge between Next.js page components and the metadata system.
 *
 * METADATA ARCHITECTURE FLOW:
 *   1. Page component calls renderDiscipleshipPlansPage(route, searchParams)
 ...
 *   import { renderDiscipleshipPlansPage } from "../metadata";
 *
 *   const { content } = await renderDiscipleshipPlansPage("discipleship-plans/list", searchParams);
 *   return <div>{content}</div>;
 *
 * ROUTE PARAMETER:
 *   The route corresponds to the XML blueprint file path under admin-community module:
 *   - "discipleship-plans/list"    -> discipleship-plans-list.xml
 *   - "discipleship-plans/profile" -> discipleship-plans-profile.xml
 *   - "discipleship-plans/manage"  -> discipleship-plans-manage.xml
 *
 * SEARCH PARAMS:
 *   URL search params are passed to service handlers via request.params.
 *   Common params include:
 *   - discipleshipPlanId: For profile/edit pages
 *   - memberId: For filtering by member
 *
 * ================================================================================
 */
...
/**
 * Renders a discipleship plans page from XML metadata.
 *
 * @param route - The page route (e.g., "discipleship-plans/list")
 * @param searchParams - URL search parameters to pass to handlers
 * @returns Object containing rendered React content
 */
export async function renderDiscipleshipPlansPage(route: string, searchParams: PageSearchParams) {
