...
-- Unified full-text search across multiple entity types
-- Uses PostgreSQL full-text search for performance
-- =====================================================

-- Create text search configuration for church-specific terms
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_ts_config WHERE cfgname = 'stewardtrack') THEN
    CREATE TEXT SEARCH CONFIGURATION stewardtrack (COPY = english);
  END IF;
END
$$;

-- Add search vectors to key tables if not exists
-- Members table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'members' AND column_name = 'search_vector'
  ) THEN
...
-- Create index for members search
CREATE INDEX IF NOT EXISTS idx_members_search_vector ON members USING GIN (search_vector);

-- Update function to maintain search vector for members
CREATE OR REPLACE FUNCTION members_search_vector_update() RETURNS trigger AS $$
BEGIN
  NEW.search_vector :=
    setweight(to_tsvector('english', coalesce(NEW.first_name, '')), 'A') ||
    setweight(to_tsvector('english', coalesce(NEW.last_name, '')), 'A') ||
    setweight(to_tsvector('english', coalesce(NEW.preferred_name, '')), 'B') ||
    setweight(to_tsvector('english', coalesce(NEW.email, '')), 'B') ||
    setweight(to_tsvector('english', coalesce(NEW.contact_number, '')), 'C') ||
    setweight(to_tsvector('english', coalesce(NEW.occupation, '')), 'C') ||
    setweight(to_tsvector('english', coalesce(NEW.address_city, '')), 'D') ||
    setweight(to_tsvector('english', coalesce(array_to_string(NEW.tags, ' '), '')), 'C');
  RETURN NEW;
END
$$ LANGUAGE plpgsql;

-- Create trigger for members search vector
  ...
  IF v_search_all OR 'member' = ANY(p_entity_types) THEN
    SELECT json_build_object(
      ...
      'totalCount', (SELECT COUNT(*) FROM members WHERE tenant_id = p_tenant_id AND (
        search_vector @@ v_search_query OR
    ...
    FROM (
      SELECT
        ...
        CASE
          WHEN m.search_vector @@ v_search_query THEN ts_rank(m.search_vector, v_search_query)
          ELSE 0.3
        END AS score
      FROM members m
      WHERE m.tenant_id = p_tenant_id
        AND (
          m.search_vector @@ v_search_query OR
          m.first_name ILIKE '%' || p_query || '%' OR
          m.last_name ILIKE '%' || p_query || '%' OR
          m.email ILIKE '%' || p_query || '%'
        )
  ...
  IF v_search_all OR 'event' = ANY(p_entity_types) THEN
    ...
    FROM (
      SELECT
        ce.id,
        ce.title,
        ce.start_at::DATE || ' at ' || ce.location AS subtitle,
        ce.description,
        NULL AS "imageUrl",
        ce.tags,
  ...
  IF v_search_all OR 'ministry' = ANY(p_entity_types) THEN
    ...
    FROM (
      SELECT
        m.id,
        m.name AS title,
        m.leader_name AS subtitle,
        m.description,
        NULL AS "imageUrl",
        ARRAY[]::TEXT[] AS tags,
        m.created_at AS timestamp,
        CASE
          WHEN m.name ILIKE p_query || '%' THEN 1.0
          WHEN m.name ILIKE '%' || p_query || '%' THEN 0.7
          ELSE 0.3
        END AS score
      FROM ministries m
      WHERE m.tenant_id = p_tenant_id
  ...
  IF v_search_all OR 'care_plan' = ANY(p_entity_types) THEN
    ...
    FROM (
      SELECT
        ...
        m.profile_picture_url AS "imageUrl",
        ARRAY[mcp.priority]::TEXT[] AS tags,
  ...
  IF v_search_all OR 'discipleship_plan' = ANY(p_entity_types) THEN
    ...
    FROM (
      SELECT
        ...
        NULL AS description,
        m.profile_picture_url AS "imageUrl",
        ARRAY[mdp.status]::TEXT[] AS tags,
        mdp.created_at AS timestamp,
        0.5 AS score
      FROM member_discipleship_plans mdp
      JOIN members m ON m.id = mdp.member_id
      LEFT JOIN discipleship_pathways dp ON dp.id = mdp.pathway_id
      WHERE m.tenant_id = p_tenant_id
        AND (
          m.first_name ILIKE '%' || p_query || '%' OR
          m.last_name ILIKE '%' || p_query || '%' OR
          dp.name ILIKE '%' || p_query || '%'
        )
...
GRANT EXECUTE ON FUNCTION global_search(UUID, TEXT, TEXT[], INTEGER, INTEGER) TO authenticated;

-- Comment for documentation
COMMENT ON FUNCTION global_search IS 'Unified global search across all entity types with full-text search support';
