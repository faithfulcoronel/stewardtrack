-- =====================================================
-- Fix Global Search RPC - Enum Type Casting
-- Fixes: invalid input value for enum financial_transaction_type
-- =====================================================
...
BEGIN
  ...
  -- Search Financial Transactions (FIXED: cast enum to TEXT)
  IF v_search_all OR 'transaction' = ANY(p_entity_types) THEN
    ...
    FROM (
      SELECT
        ...
        ft.date || ' - ' || COALESCE(ft.type::TEXT, 'Transaction') AS subtitle,
        CASE WHEN ft.debit > 0 THEN 'Debit: ' || ft.debit ELSE 'Credit: ' || ft.credit END AS description,
        NULL AS "imageUrl",
        ARRAY[COALESCE(ft.type::TEXT, 'Transaction')]::TEXT[] AS tags,
  ...
  -- Search Ministries (use leader_id with join, not leader_name)
  ...
  -- Search Discipleship Plans (pathway column, not pathway_id)
...
COMMENT ON FUNCTION global_search IS 'Unified global search across all entity types - v3 with proper enum casting';
