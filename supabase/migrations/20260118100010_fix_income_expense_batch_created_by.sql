-- =============================================================================
-- Migration: Fix Income/Expense Batch RPC - Add created_by to GL entries
-- =============================================================================
-- Adds created_by column to financial_transactions inserts
-- =============================================================================
...
BEGIN
  -- Use provided created_by or get current user from auth context
  v_effective_created_by := COALESCE(p_created_by, auth.uid());
  ...
  IF v_last_number IS NOT NULL THEN
    v_sequence := COALESCE(NULLIF(SPLIT_PART(v_last_number, '-', 3), '')::INTEGER, 0) + 1;
  ELSE
    v_sequence := 1;
  END IF;
  ...
  INSERT INTO financial_transaction_headers (
    ...
    reference, source_id, status, created_by
  )
  VALUES (
    p_tenant_id, v_transaction_number, p_transaction_date, p_description,
    p_reference, p_source_id, 'draft', v_effective_created_by
  )
  ...
  FOR v_coa_record IN
    ...
    WHERE c.tenant_id = p_tenant_id
      AND c.id IN (
        ...
        WHERE line->>'category_id' IS NOT NULL AND line->>'category_id' != ''
      )
  LOOP
  ...
  FOR v_coa_record IN
    ...
    WHERE fs.tenant_id = p_tenant_id
      AND fs.id IN (
        ...
        FROM jsonb_array_elements(p_lines) AS line
        WHERE line->>'source_id' IS NOT NULL AND line->>'source_id' != ''
      )
  LOOP
  ...
  FOR v_coa_record IN
    ...
    WHERE f.tenant_id = p_tenant_id
      AND f.id IN (
        ...
        FROM jsonb_array_elements(p_lines) AS line
        WHERE line->>'fund_id' IS NOT NULL AND line->>'fund_id' != ''
      )
  LOOP
  ...
  LOOP
    ...
    -- Create GL entries with created_by
    IF v_transaction_type = 'income' THEN
      INSERT INTO financial_transactions (
        tenant_id, type, account_id, fund_id, source_id, category_id,
        coa_id, date, description, debit, credit, batch_id, header_id, created_by
      )
      VALUES (
        p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id_line, v_category_id,
        v_source_coa_id, p_transaction_date, v_line_description, v_amount, 0, v_batch_id, v_header_id, v_effective_created_by
      )
      ...
      VALUES (
        p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id_line, v_category_id,
        v_category_coa_id, p_transaction_date, v_line_description, 0, v_amount, v_batch_id, v_header_id, v_effective_created_by
      )
    ...
    ELSIF v_transaction_type = 'opening_balance' THEN
      ...
      VALUES (
        p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id_line, v_category_id,
        v_fund_coa_id, p_transaction_date, v_line_description, 0, v_amount, v_batch_id, v_header_id, v_effective_created_by
      )
      RETURNING id INTO v_credit_tx_id;
    ...
    ELSE
      ...
      VALUES (
        p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id_line, v_category_id,
        v_category_coa_id, p_transaction_date, v_line_description, v_amount, 0, v_batch_id, v_header_id, v_effective_created_by
      )
      RETURNING id INTO v_debit_tx_id;
      ...
      VALUES (
        p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id_line, v_category_id,
        v_source_coa_id, p_transaction_date, v_line_description, 0, v_amount, v_batch_id, v_header_id, v_effective_created_by
      )
      RETURNING id INTO v_credit_tx_id;
    ...
    INSERT INTO income_expense_transactions (
      tenant_id, transaction_type, transaction_date, amount, description,
      reference, category_id, fund_id, source_id, account_id, header_id, line, created_by
    )
    VALUES (
      p_tenant_id, v_transaction_type::financial_transaction_type, p_transaction_date, v_amount, v_line_description,
      p_reference, v_category_id, v_fund_id, v_source_id_line, v_account_id, v_header_id, v_line_number, v_effective_created_by
    )
    RETURNING id INTO v_ie_tx_id;

    -- Create mapping
    INSERT INTO income_expense_transaction_mappings (
      tenant_id, transaction_id, transaction_header_id, debit_transaction_id, credit_transaction_id
    )
    VALUES (
      p_tenant_id, v_ie_tx_id, v_header_id, v_debit_tx_id, v_credit_tx_id
    );
