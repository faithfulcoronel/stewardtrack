-- =============================================================================
-- Migration: Create import_members_batch RPC function
-- =============================================================================
-- Description: Batch import members from Excel with validation
-- Date: 2026-01-19
-- =============================================================================
...
-- =============================================================================
-- RPC Function: import_members_batch
-- =============================================================================
-- Input Parameters:
--   p_tenant_id: UUID - The tenant to import members into
--   p_user_id: UUID - The user performing the import
--   p_members: JSONB - Array of member objects to import
--
...
--   imported_ids: array of created member UUIDs
-- =============================================================================
    ...
    BEGIN
      ...
      IF v_membership_stage_code IS NOT NULL AND v_membership_stage_code != '' THEN
        ...
        IF v_membership_stage_id IS NULL THEN
          -- Try to find a default membership stage
          SELECT id INTO v_membership_stage_id
          FROM membership_stage
          WHERE tenant_id = p_tenant_id
            AND deleted_at IS NULL
...
GRANT EXECUTE ON FUNCTION import_members_batch(UUID, UUID, JSONB) TO authenticated;
...
DO $$
BEGIN
  RAISE NOTICE 'Created import_members_batch RPC function successfully';
END $$;
