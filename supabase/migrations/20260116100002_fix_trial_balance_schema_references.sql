...
-- Uses income_expense_transactions table (which has source_id and fund_id)
-- instead of financial_transactions table
-- Fixes:
-- 1. Use income_expense_transactions.transaction_type instead of non-existent entry_type
-- 2. Source view: JOIN to financial_sources using source_id
-- 3. Fund view: JOIN to funds using fund_id
...
BEGIN
  ...
  ELSIF p_view_by = 'source' THEN
    -- Source view: Group by financial_sources using income_expense_transactions.source_id
    RETURN QUERY
    WITH periods AS (
      SELECT fp.id AS period_id, fp.name AS period_name, fp.start_date, fp.end_date
      FROM fiscal_periods fp
    ...
    source_period_totals AS (
      SELECT
        fs.id AS source_id,
        COALESCE(fs.account_number, SUBSTRING(fs.id::text, 1, 8)) AS source_code,
        fs.name AS source_name,
        fs.source_type AS source_type,
        p.period_id,
        p.period_name,
        -- For sources: expense transactions are debits, income transactions are credits
        COALESCE(SUM(CASE WHEN iet.transaction_type = 'expense' THEN iet.amount ELSE 0 END), 0) AS debit,
        COALESCE(SUM(CASE WHEN iet.transaction_type = 'income' THEN iet.amount ELSE 0 END), 0) AS credit
      FROM financial_sources fs
      CROSS JOIN periods p
      LEFT JOIN income_expense_transactions iet
        ON iet.source_id = fs.id
        AND iet.tenant_id = p_tenant_id
        AND iet.transaction_date BETWEEN p.start_date AND p.end_date
        AND iet.deleted_at IS NULL
  ...
  ELSIF p_view_by = 'fund' THEN
    -- Fund view: Group by funds using income_expense_transactions.fund_id
    RETURN QUERY
    WITH periods AS (
      SELECT fp.id AS period_id, fp.name AS period_name, fp.start_date, fp.end_date
      FROM fiscal_periods fp
    ...
    fund_period_totals AS (
      SELECT
        ...
        p.period_id,
        p.period_name,
        -- For funds: expense transactions are debits, income transactions are credits
        COALESCE(SUM(CASE WHEN iet.transaction_type = 'expense' THEN iet.amount ELSE 0 END), 0) AS debit,
        COALESCE(SUM(CASE WHEN iet.transaction_type = 'income' THEN iet.amount ELSE 0 END), 0) AS credit
      FROM funds f
      CROSS JOIN periods p
...
COMMENT ON FUNCTION report_trial_balance_by_period(uuid, uuid, text) IS
  'Enhanced trial balance report showing period-by-period breakdown. Uses income_expense_transactions table. Supports viewing by category, source, or fund.';
