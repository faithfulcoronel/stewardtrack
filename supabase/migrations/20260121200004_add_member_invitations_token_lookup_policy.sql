-- =============================================================================
-- Add function for public invitation token lookup
-- =============================================================================
-- Description: The existing RLS policy only allows admin users to view
-- member_invitations. However, when a user clicks an invitation link,
-- they're not authenticated yet and need to see the invitation details.
--
-- This migration creates a SECURITY DEFINER function that bypasses RLS
-- to look up invitation details by token. This is safe because:
-- 1. The token is the secret - knowing the token is proof of access
-- 2. Only pending/sent invitations are returned
...
-- Create function to lookup invitation by token (bypasses RLS)
...
COMMENT ON FUNCTION get_invitation_by_token(text) IS 'Looks up invitation details by token, bypassing RLS. Safe because token is the secret.';
...
DO $$
BEGIN
  RAISE NOTICE 'Migration completed: added get_invitation_by_token function for public invitation lookup';
END $$;
