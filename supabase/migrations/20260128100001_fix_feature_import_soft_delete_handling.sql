-- =============================================================================
-- Migration: Fix Feature Import Soft Delete Handling
-- =============================================================================
-- Fixes the import_features_batch RPC function to properly handle soft-deleted
-- features. Previously, the function only looked for features where
-- deleted_at IS NULL, but the unique constraint on `code` doesn't consider
-- deleted_at. This caused INSERT failures when trying to add features that
-- were previously soft-deleted.
--
-- Changes:
-- - Look for existing features by code regardless of deleted_at status
-- - If found (even soft-deleted), update and "undelete" by setting deleted_at = NULL
-- - Only INSERT if the feature truly doesn't exist in the database
-- =============================================================================
...
DECLARE
  ...
  v_existing_feature_deleted BOOLEAN;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      IF v_action = 'update' THEN
        ...
        END IF;
      ELSE
        -- Add new feature: check for existing by code INCLUDING soft-deleted records
        SELECT id, (deleted_at IS NOT NULL) INTO v_existing_feature_id, v_existing_feature_deleted
        FROM feature_catalog
        WHERE code = v_feature->>'code';

        IF v_existing_feature_id IS NOT NULL THEN
          -- Feature exists (active or soft-deleted) - update it and undelete if necessary
          UPDATE feature_catalog
          SET
            name = COALESCE(NULLIF(v_feature->>'name', ''), name),
            description = COALESCE(v_feature->>'description', description),
          ...
          -- If it was soft-deleted and we're re-adding it, count as added
          -- Otherwise count as updated
          IF v_existing_feature_deleted THEN
            v_features_added := v_features_added + 1;
          ELSE
            v_features_updated := v_features_updated + 1;
          END IF;
        ELSE
          -- Feature doesn't exist at all - insert new
  ...
  LOOP
    ...
    BEGIN
      -- Get feature_id by code (only active features can have permissions added)
  ...
  LOOP
    ...
    BEGIN
      -- Get permission_id by feature_code + permission_code (only active features)
...
COMMENT ON FUNCTION import_features_batch IS
  ...
  'Properly handles soft-deleted features by undeleting them when re-adding. '
