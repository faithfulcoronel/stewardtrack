...
BEGIN
  ...
  IF NOT (SELECT is_admin()) AND NOT EXISTS (
    ...
    JOIN public.role_permissions rp ON ur2.role_id = rp.role_id
  ...
  permission_data AS (
    ...
    FROM role_data rd
    LEFT JOIN public.role_permissions rp ON rd.id = rp.role_id
  ...
  CASE operation
    ...
    WHEN 'create' THEN
      ...
      IF user_data ? 'roles' AND jsonb_array_length(user_data->'roles') > 0 THEN
        ...
        JOIN public.roles r ON r.id::text = role_id
    ...
    WHEN 'update' THEN
      ...
      IF user_data ? 'roles' THEN
        ...
        WHERE ur.user_id = target_user_id
        AND ur.role_id != (SELECT id FROM public.roles WHERE name = 'member');
    ...
    WHEN 'delete' THEN
      ...
      DELETE FROM public.user_roles ur WHERE ur.user_id = target_user_id;
