-- =============================================================================
-- Migration: Add encryption tracking to import_members_batch RPC function
-- =============================================================================
-- Description: Update RPC function to set encrypted_fields and encryption_key_version
--              when importing members with pre-encrypted PII fields
-- Date: 2026-01-20
-- =============================================================================
...
-- =============================================================================
-- RPC Function: import_members_batch (with encryption tracking)
-- =============================================================================
...
DECLARE
  ...
  v_encrypted_fields TEXT[] := ARRAY['first_name', 'last_name', 'middle_name', 'email', 'contact_number'];
BEGIN
  ...
  LOOP
    ...
    BEGIN
      -- Extract required fields (these may be encrypted)
      v_first_name := TRIM(v_member->>'first_name');
      v_last_name := TRIM(v_member->>'last_name');
      v_email := TRIM(v_member->>'email');

      -- Skip if both names are empty (encrypted values will have content)
      ...
      -- Skip email duplicate check for encrypted emails (they won't match anyway)
      -- The check would need to happen at the application layer for encrypted data
      ...
      -- Normalize gender value (only if not encrypted)
      v_gender := TRIM(v_member->>'gender');
      IF v_gender IS NOT NULL AND v_gender != '' THEN
        -- Check if it looks like a non-encrypted value
        IF v_gender ~ '^(male|female|other)$' THEN
          v_gender := LOWER(v_gender);
        ELSE
          -- Might be encrypted or invalid, set to null
          v_gender := NULL;
        END IF;
      ELSE
        v_gender := NULL;
      ...
      -- Normalize marital status value (only if not encrypted)
      v_marital_status := TRIM(v_member->>'marital_status');
      IF v_marital_status IS NOT NULL AND v_marital_status != '' THEN
        -- Check if it looks like a non-encrypted value
        IF v_marital_status ~ '^(single|married|widowed|divorced|engaged)$' THEN
          v_marital_status := LOWER(v_marital_status);
        ELSE
          -- Might be encrypted or invalid, set to null
      ...
      -- Insert the member with encryption tracking fields
      INSERT INTO members (
        tenant_id,
        first_name,
        last_name,
        ...
        -- Cast gender to proper enum type
        CASE
          WHEN v_gender IS NOT NULL
          THEN v_gender::gender_type
          ELSE NULL
        END,
        -- Cast marital_status to proper enum type
        CASE
          WHEN v_marital_status IS NOT NULL
          THEN v_marital_status::marital_status_type
          ELSE NULL
        END,
        CASE
        ...
        CASE
          ...
          ELSE NULL
        END,
        -- Set encrypted_fields to track which PII fields are encrypted
        v_encrypted_fields,
        -- Set encryption_key_version (will be updated by key manager if needed)
        1,
        NOW(),
        NOW()
      )
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Record error with detailed message and continue
      v_errors := v_errors || jsonb_build_array(
...
DO $$
BEGIN
  RAISE NOTICE 'Updated import_members_batch RPC function - added encryption tracking (encrypted_fields, encryption_key_version)';
END $$;
