-- =============================================================================
-- Migration: Fix Update Income/Expense Batch RPC - Cast status to enum
-- =============================================================================
-- Fixes the status column update to cast text to transaction_status enum
-- =============================================================================
...
BEGIN
  ...
  IF p_header_update IS NOT NULL AND p_header_update != '{}'::JSONB THEN
    -- Extract new status if provided
    v_new_status := p_header_update->>'status';
    ...
    SET
      ...
      status = CASE
        WHEN v_new_status IS NOT NULL THEN v_new_status::transaction_status
        ELSE status
      END,
  ...
  LOOP
    ...
    IF v_line_id IS NOT NULL AND v_existing_mappings ? v_line_id::TEXT THEN
      -- -----------------------------------------------------------------------
      -- PASS 2: Update dirty existing lines (with type cast to enum)
      -- -----------------------------------------------------------------------
      IF v_is_dirty THEN
        v_debit_tx_id := (v_existing_mappings->v_line_id::TEXT->>'debit_transaction_id')::UUID;
        v_credit_tx_id := (v_existing_mappings->v_line_id::TEXT->>'credit_transaction_id')::UUID;
        ...
        IF v_transaction_type = 'income' THEN
          ...
          IF v_debit_tx_id IS NOT NULL THEN
            ...
            SET type = v_transaction_type::financial_transaction_type, account_id = v_account_id, fund_id = v_fund_id,
        ...
        SET transaction_type = v_transaction_type::financial_transaction_type, transaction_date = v_transaction_date,
            ...
            line = v_line_number, updated_by = v_effective_updated_by, updated_at = NOW()
    ...
    ELSE
      -- -----------------------------------------------------------------------
      -- PASS 3: Insert new lines (with type cast and created_by)
      -- -----------------------------------------------------------------------
      ...
      IF v_transaction_type = 'income' THEN
        ...
        VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id, v_category_id,
          v_source_coa_id, v_transaction_date, v_line_description, v_amount, 0, v_batch_id, p_header_id, v_effective_updated_by)
        RETURNING id INTO v_debit_tx_id;

        INSERT INTO financial_transactions (tenant_id, type, account_id, fund_id, source_id, category_id,
          coa_id, date, description, debit, credit, batch_id, header_id, created_by)
        VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id, v_category_id,
          v_category_coa_id, v_transaction_date, v_line_description, 0, v_amount, v_batch_id, p_header_id, v_effective_updated_by)
      ...
      ELSIF v_transaction_type = 'opening_balance' THEN
        ...
        INSERT INTO financial_transactions (tenant_id, type, account_id, fund_id, source_id, category_id,
          coa_id, date, description, debit, credit, batch_id, header_id, created_by)
        VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id, v_category_id,
          v_fund_coa_id, v_transaction_date, v_line_description, 0, v_amount, v_batch_id, p_header_id, v_effective_updated_by)
        RETURNING id INTO v_credit_tx_id;
      ...
      ELSE
        ...
        INSERT INTO financial_transactions (tenant_id, type, account_id, fund_id, source_id, category_id,
          coa_id, date, description, debit, credit, batch_id, header_id, created_by)
        VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id, v_category_id,
          v_category_coa_id, v_transaction_date, v_line_description, v_amount, 0, v_batch_id, p_header_id, v_effective_updated_by)
        RETURNING id INTO v_debit_tx_id;

        INSERT INTO financial_transactions (tenant_id, type, account_id, fund_id, source_id, category_id,
          coa_id, date, description, debit, credit, batch_id, header_id, created_by)
        VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_account_id, v_fund_id, v_source_id, v_category_id,
          v_source_coa_id, v_transaction_date, v_line_description, 0, v_amount, v_batch_id, p_header_id, v_effective_updated_by)
        RETURNING id INTO v_credit_tx_id;
      END IF;

      v_gl_entries_affected := v_gl_entries_affected + 2;

      -- Create IE transaction (with type cast)
      ...
      VALUES (p_tenant_id, v_transaction_type::financial_transaction_type, v_transaction_date, v_amount, v_line_description,
        v_reference, v_category_id, v_fund_id, v_source_id, v_account_id, p_header_id, v_line_number, v_effective_updated_by)
...
COMMENT ON FUNCTION update_income_expense_batch IS
  ...
  'Casts status to transaction_status enum. '
  'Includes created_by for new GL entries.';
