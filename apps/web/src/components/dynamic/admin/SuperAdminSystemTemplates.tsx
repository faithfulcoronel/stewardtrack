...
export interface SuperAdminSystemTemplatesProps {
  title?: string;
  description?: string;
}
...
const SYSTEM_EVENT_TYPES = [
  { value: 'system.welcome', label: 'System Welcome' },
  { value: 'system.password_reset', label: 'Password Reset' },
  { value: 'system.email_verification', label: 'Email Verification' },
  { value: 'system.account_locked', label: 'Account Locked' },
  { value: 'system.subscription_created', label: 'Subscription Created' },
  { value: 'system.subscription_renewed', label: 'Subscription Renewed' },
  { value: 'system.subscription_cancelled', label: 'Subscription Cancelled' },
  { value: 'system.payment_received', label: 'Payment Received' },
  { value: 'system.payment_failed', label: 'Payment Failed' },
  { value: 'system.trial_ending', label: 'Trial Ending' },
  { value: 'system.maintenance', label: 'Maintenance Notice' },
...
export function SuperAdminSystemTemplates({
  title = 'System Templates',
  description = 'Manage global notification templates that apply to all tenants',
}: SuperAdminSystemTemplatesProps) {
  const [loading, setLoading] = useState(true);
  const [templates, setTemplates] = useState<SystemTemplate[]>([]);
  const [saving, setSaving] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [templateToDelete, setTemplateToDelete] = useState<SystemTemplate | null>(null);
  ...
  const handleDuplicate = (template: SystemTemplate) => {
    setEditingTemplate(null);
    setFormData({
      event_type: template.event_type,
      channel: template.channel,
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingTemplate
        ? `/api/system-settings/templates/${editingTemplate.id}`
        : '/api/system-settings/templates';
      const method = editingTemplate ? 'PATCH' : 'POST';

      const response = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, is_system: true }),
      });
      ...
      toast({
        ...
        description: `System template "${formData.name}" has been ${editingTemplate ? 'updated' : 'created'}`,
      });
  ...
  const handleDeleteClick = (template: SystemTemplate) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/system-settings/templates/${templateToDelete.id}`, {
        method: 'DELETE',
      });
      ...
      toast({
        ...
        description: 'The system template has been deleted',
      });
        ...
        <CardContent className="py-4">
          <div className="flex gap-3">
            ...
            <div className="text-sm">
              <p className="font-medium text-amber-700 dark:text-amber-500 mb-1">Global Templates</p>
              <p className="text-muted-foreground">
                These templates serve as system defaults for all tenants. Tenants with Enterprise tier can
                override these with their own customizations.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {templates.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="font-medium mb-1">No System Templates</h3>
            <p className="text-sm text-muted-foreground max-w-sm mb-4">
              Create global templates for system-wide notifications.
            </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingTemplate
                ? 'Update the global notification template'
                : 'Create a new system-wide notification template'
              }
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="name"
                placeholder="e.g., Welcome Email - Default"
                value={formData.name}
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Common variables: {"{{tenant_name}}"}, {"{{user_name}}"}, {"{{user_email}}"}, {"{{app_url}}"}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Default templates are used when tenants don't have their own
                </p>
              </div>
              <Switch
                id="is_default"
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className="max-w-[90vw] sm:max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the system template "{templateToDelete?.name}".
              This may affect notification delivery across all tenants. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
