...
const TIER_CONFIG: Record<string, { icon: React.ReactNode; gradient: string; color: string }> = {
  essential: {
...
export function PlanComparisonMatrix({
  ...
  return (
    <motion.div
      ...
      className={`relative ${hasFeaturedOffering ? 'pt-5' : ''}`}
    >
      ...
      {sortedOfferings.map((offering, index) => {
        ...
        // Label column is 1.5fr, plan columns are 1fr each
        const labelFr = 1.5;
        const planFr = 1;
        const totalFr = labelFr + (sortedOfferings.length * planFr);

        // Position = center of the featured column
        // Start of column in fr = labelFr + (index * planFr)
        // Center of column in fr = start + (planFr / 2)
