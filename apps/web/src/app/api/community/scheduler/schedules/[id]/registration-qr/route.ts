...
import { encodeRegistrationToken } from '@/lib/tokens/registrationToken';
import { getCurrentTenantId } from '@/lib/server/context';

type RouteParams = { params: Promise<{ id: string }> };

/**
 * GET /api/community/scheduler/schedules/[id]/registration-qr
 * Get or generate the registration QR token for a schedule
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Verify schedule exists and has registration required
    ...
    if (!schedule.registration_required) {
      return NextResponse.json(
        { success: false, error: 'Registration is not enabled for this schedule' },
        { status: 400 }
      );
    }

    // Generate a token valid for 7 days by default (can be regenerated)
    const expirationHours = 168; // 7 days
...
/**
 * POST /api/community/scheduler/schedules/[id]/registration-qr
 * Generate a new registration QR token with custom expiration
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (body.expires_at) {
      ...
      // Hours provided (default: 168 = 7 days)
      expirationHours = body.expires_in_hours || 168;

      // Validate expiration range (1 hour to 720 hours / 30 days)
      if (expirationHours < 1 || expirationHours > 720) {
        return NextResponse.json(
          { success: false, error: 'Expiration must be between 1 and 720 hours (30 days)' },
          { status: 400 }
        );
      }
