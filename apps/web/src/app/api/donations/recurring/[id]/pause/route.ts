...
/**
 * POST /api/donations/recurring/[id]/pause
 *
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const updatedDonation = await recurringDonationService.pauseRecurringDonation(
      donationId,
      tenantId
    );
    ...
    console.error('[API] Error pausing recurring donation:', error);
    const message = error instanceof Error ? error.message : 'Failed to pause recurring donation';
    return NextResponse.json(
      { error: message },
      { status: 400 }
    );
