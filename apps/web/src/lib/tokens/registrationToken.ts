 ...
 * for QR code-based event registration.
 *
 * Token format: Encodes tenant_id + schedule_id + expiration into a URL-safe string
 *
 * Usage:
 * - encodeRegistrationToken(tenantId, scheduleId, expirationDays) → "encoded-token"
 * - decodeRegistrationToken(token) → { tenantId, scheduleId, expiresAt }
 *
 * Security:
...
const SECRET_KEY = process.env.REGISTRATION_TOKEN_SECRET || process.env.SHORT_URL_TOKEN_SECRET || "stewardtrack-registration-2024";
...
/**
 * Encode a registration token that includes tenant ID, schedule ID, and expiration
 *
 * @param tenantId - The tenant's UUID
 * @param scheduleId - The schedule's UUID
 * @param expirationDays - Number of days until the token expires (default: 30)
 * @returns A URL-safe token string
 *
 * @example
 * encodeRegistrationToken("tenant-uuid", "schedule-uuid", 30)
 * // Returns: "XYZabc123..."
 */
export function encodeRegistrationToken(
  ...
  // Combine: "R" prefix (for registration) + compact tenant ID + ":" + compact schedule ID + "|" + expiration timestamp
  const payload = `R${compactTenant}:${compactSchedule}|${expiresAt}`;
...
/**
 * Decode a registration token back to tenant ID, schedule ID, and expiration
 *
 * @param token - The encoded token
 * @returns The decoded data with tenant ID, schedule ID, and expiration, or null if invalid
 *
 * @example
 * decodeRegistrationToken("XYZabc123...")
 * // Returns: { tenantId: "...", scheduleId: "...", expiresAt: 1234567890000 }
 */
export function decodeRegistrationToken(token: string): RegistrationTokenData | null {
  try {
    // Base64url decode
    ...
    // Check for registration token prefix
...
export function isRegistrationTokenValid(token: string): boolean {
  const data = decodeRegistrationToken(token);
  if (!data) return false;
  return Date.now() < data.expiresAt;
}

/**
 * Get the remaining validity of a registration token
 *
 * @param token - The encoded token
 * @returns Number of days remaining, or -1 if invalid/expired
 */
export function getRegistrationTokenDaysRemaining(token: string): number {
  const data = decodeRegistrationToken(token);
  if (!data) return -1;
...
/**
 * Generate the full registration URL for a schedule
 *
 * @param tenantId - The tenant's UUID
 * @param scheduleId - The schedule's UUID
 * @param expirationDays - Number of days until the token expires (default: 30)
 * @param baseUrl - The base URL of the application
 * @returns The full registration URL
 *
 * @example
 * getRegistrationUrl("tenant-uuid", "schedule-uuid", 30)
 * // Returns: "https://stewardtrack.com/register/XYZabc123..."
 */
export function getRegistrationUrl(
  ...
  const token = encodeRegistrationToken(tenantId, scheduleId, expirationDays);
